...
<div class="card card-default">
	...
	<!-- /.card-header -->
	<!-- form start -->
	<form role="form" action="{{ route('client-bookings.update', ['booking' => $booking]) }}" method="post">
		@method('PATCH')
		@csrf
		<div class="card-body row">
			<div class="col-md-6">
				...
				<div class="form-group">
					...
					<select class="form-control form-control-sm @error('service_id') is-invalid @enderror" name="service_id" id="service_id">
						...
						@foreach($services as $service)
							<option value="{{ $service->id }}"{{ $service->id ? 'selected' : '' }}>{{ $service->title }}</option>
						@endforeach
					</select>
					@error('service_id')
				...
				<div class="form-group">
					...
					<input type="date" class="form-control form-control-sm @error('booking_date') is-invalid @enderror" name="booking_date" id="booking_date" placeholder="Enter booking_date" value="{{ old('booking_date') ?? $booking->booking_date }}">
					@error('booking_date')
					<div class="error-alert">{{ $message }}</div>
					@enderror
				</div>
				...
				<div class="form-group">
					...
					<textarea class="form-control" rows="3" placeholder="Enter comment" name="comment">{{ $booking->comment }}</textarea>
				</div>
				<!-- /.description-->
			...
			<div class="col-md-6">
				...
				<div class="form-group">
					...
					<input type="number" class="form-control form-control-sm @error('quantity') is-invalid @enderror"
						name="quantity" id="quantity" placeholder="Enter booking quantity" value="{{ old('quantity') ?? $booking->quantity }}">
					@error('quantity')
					<div class="error-alert">{{ $message }}</div>
					@enderror
				</div>
