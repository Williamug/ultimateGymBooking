...
@section('content')
	<div class="card card-default">
		...
		<div class="card-body">
			<form action="{{ route('instructor-post.update', ['tip' => $tip]) }}" method="post" enctype="multipart/form-data">
				@csrf
				@method('PATCH')
				<div>
					<div class="form-group">
		                <label>Tip of the day</label>
		                <textarea class="form-control" rows="3" placeholder="Enter ..." name="post">{{old('post') ?? $tip->post}}</textarea>
		                @error('post')
