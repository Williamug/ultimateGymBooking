			...
			<div class="card-body box-profile">
				<div class="text-center">
					@if(Auth::user()->instructor->profile_image != null)
						<img class="profile-user-img img-fluid img-circle" src="{{ asset('storage/' . Auth::user()->instructor->profile_image) }}" alt="User profile picture">
					@else
				...
				<ul class="list-group list-group-unbordered mb-3">
					<li class="list-group-item">
						<b>Phone Number:</b> <a class="float-right">{{ Auth::user()->instructor->phone_number }}</a>
					</li>
					<li class="list-group-item">
						<b>Gender:</b> <a class="float-right">{{ Auth::user()->instructor->gender }}</a>
					</li>
					<li class="list-group-item">
						<b>Date of birth:</b> <a class="float-right">{{ Carbon\Carbon::parse(Auth::user()->instructor->dob,)->format('jS M Y') }}</a>
					<li class="list-group-item">
				...
				<div class="tab-content">
					<div class="active tab-pane" id="booking">
						...
						<div class="card card-default">
							<!-- form start -->
							<form role="form" action="{{ route('instructor-profile.update', ['user' => $user]) }}" method="post" enctype="multipart/form-data">
								@method('PATCH')
								@csrf
								<div class="card-body row">
									<div class="col-md-6">
										...
										<div class="form-group">
											...
											<input type="date" class="form-control form-control-sm" name="dob" id="dob" placeholder="Enter date of birth" value="{{ old('dob') ?? Auth::user()->instructor->dob}}">
										</div>
										<!-- date of birth-->
									...
									<div class="col-md-6">
										...
										<div class="form-group">
											...
											<div class="custom-control custom-radio">
												<input type="radio" id="male" name="gender" value="Male" {{ Auth::user()->instructor->gender == 'Male' ? 'checked' : ''}} checked="checked" class="custom-control-input">
												<label for="male" class="custom-control-label">Male</label>
											</div>
											<div class="custom-control custom-radio">
												<input type="radio" id="female" name="gender" value="Female" {{ Auth::user()->instructor->gender == 'Female' ? 'checked' : ''}} class="custom-control-input">
										...
										<div class="form-group">
											...
											<input type="text" class="form-control form-control-sm" name="phone_number" id="phone_number" placeholder="Enter phone number" value="{{ old('phone_number') ?? Auth::user()->instructor->phone_number}}">
										</div>
										<!-- /.phone_number-->
									</div>
								</div>
